...
const STEPS = [
    ...
    },
    {
        ...
        bg: "bg-indigo-100",
        description:
            "Créez un code PIN de 4 à 8 chiffres pour accéder rapidement à vos mots de passe sans retaper votre mot de passe principal.",
    },
    {
        id: "biometric",
        label: "Biométrie",
...
export default function OnboardingPage() {
    ...
    const handleBioRegister = async () => {
        ...
        try {
            ...
            if (!optRes.ok) throw new Error("Impossible d'initialiser l'enregistrement.");
            ...
            if (!verifyRes.ok) throw new Error(verifyData.error || "Enregistrement échoué.");

            // Store device credential ID locally
            if (verifyData.credentialDbId) {
                const stored = JSON.parse(localStorage.getItem("wa_device_cred_ids") || "[]");
        ...
        return (
            <div className="min-h-screen bg-linear-to-br from-indigo-50 via-white to-purple-50 flex items-center justify-center p-4">
                <div className="w-full max-w-lg text-center">
                    ...
                    <Button onClick={() => setStepIndex(0)} className="w-full flex items-center justify-center gap-2 text-base py-3">
        ...
        return (
            <div className="min-h-screen bg-linear-to-br from-indigo-50 via-white to-purple-50 flex items-center justify-center p-4">
                <div className="w-full max-w-md text-center">
                        ...
                        {STEPS.map((s) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <span className={`text-sm font-medium ${done ? "text-green-700" : "text-gray-400"}`}>
                                        {s.label}
                                    </span>
                                    {done ? (
                                        <CheckCircle className="w-4 h-4 text-green-500 ml-auto" />
                                    ) : (
                                        <span className="text-xs text-gray-400 ml-auto">Ignoré</span>
                                    )}
                                </div>
                            );
                        })}
                    ...
                    <Button onClick={handleFinish} className="w-full flex items-center justify-center gap-2 text-base py-3">
        ...
        <div className="min-h-screen bg-linear-to-br from-indigo-50 via-white to-purple-50 flex items-center justify-center p-4">
            <div className="w-full max-w-md">
                ...
                <div className="flex items-center justify-center gap-3 mb-8">
                    {STEPS.map((s, i) => (
                        <div key={s.id} className="flex items-center gap-3">
                            <StepDot index={i} />
                            {i < STEPS.length - 1 && (
                                <div className={`h-0.5 w-10 ${i < stepIndex ? "bg-indigo-400" : "bg-gray-200"}`} />
                            )}
                        </div>
                    ))}
                </div>
                ...
                <div className="bg-white rounded-2xl shadow-lg p-6 sm:p-8">
                        ...
                        <div>
                            {tfaState === "idle" && (
                                <>
                                    ...
                                    <Button onClick={handleTfaSetup} className="w-full flex items-center justify-center gap-2">
                                        <QrCode className="w-4 h-4" />
                                        Activer le 2FA
                                    </Button>
                                    {tfaError && <p className="mt-3 text-sm text-red-600">{tfaError}</p>}
                                </>
                            )}

                            {tfaState === "loading" && (
                                <p className="text-center text-gray-500 py-4">Génération du QR code…</p>
                            )}

                            {(tfaState === "setup" || tfaState === "verifying") && (
                                <>
                                    ...
                                    {tfaQr && (
                                        <div className="flex justify-center mb-4">
                                            <Image src={tfaQr} alt="QR Code 2FA" width={160} height={160} className="rounded-lg border border-gray-200" />
                                        </div>
                                    )}
                                    <div className="bg-gray-50 rounded-lg p-3 mb-4">
                                        <p className="text-xs text-gray-500 mb-1">Code manuel :</p>
                                        <p className="font-mono text-sm text-gray-800 break-all">{tfaSecret}</p>
                                    </div>
                                    <form onSubmit={handleTfaEnable} className="space-y-3">
                                        <Input
                                            type="text"
                                            inputMode="numeric"
                                        ...
                                        <Button
                                            type="submit"
                                            disabled={tfaCode.length !== 6 || tfaState === "verifying"}
                                            className="w-full"
                                        >
                                            {tfaState === "verifying" ? "Vérification…" : "Confirmer"}
                                        </Button>
                                    </form>
                                </>
                            )}
                            ...
                            {tfaState === "done" && (
                                <div className="text-center py-4">
                                    ...
                                    <p className="font-semibold text-gray-800 mb-1">2FA activé !</p>
                                    <p className="text-sm text-gray-500 mb-5">Votre compte est maintenant doublement protégé.</p>
                                    <Button onClick={next} className="w-full flex items-center justify-center gap-2">
                    ...
                    {currentStep.id === "pin" && (
                        <div>
                            <p className="text-gray-600 text-sm mb-5">{currentStep.description}</p>
                            <form onSubmit={handlePinSubmit} className="space-y-4">
                                ...
                                <Button
                                    type="submit"
                                    disabled={pinLoading || pin.length < 4 || !currentPassword}
                                    className="w-full"
                                >
                                    {pinLoading ? "Configuration…" : "Créer mon PIN"}
                                </Button>
                            </form>
                        </div>
                    )}
                        ...
                        <div>
                            {!bioSupported ? (
                                <>
                                    ...
                                    <Button onClick={next} variant="secondary" className="w-full">
                                ...
                                <>
                                    ...
                                    {bioError && <p className="mb-3 text-sm text-red-600">{bioError}</p>}
                                    <Button
                                    ...
                                    >
                                        ...
                                        {bioLoading ? "Enregistrement…" : "Enregistrer ma biométrie"}
                                    </Button>
                                </>
                            )}
                        </div>
                    ...
                    {!completedSteps.includes(currentStep.id) && tfaState !== "done" && (
                        <button
                            onClick={next}
                            disabled={pinLoading}
                            className="mt-5 w-full text-sm text-gray-400 hover:text-gray-600 underline underline-offset-2"
                        >
                            Passer cette étape →
                        </button>
                    )}
