...
const STRENGTH_LABELS = [
    { label: "", color: "", bar: 0 },
    { label: "Très faible — à changer immédiatement", color: "text-red-700", bar: 20, barColor: "bg-red-500", icon: "🔴" },
    { label: "Faible — facile à deviner", color: "text-orange-700", bar: 40, barColor: "bg-orange-400", icon: "🟠" },
    { label: "Moyen — peut être amélioré", color: "text-yellow-700", bar: 60, barColor: "bg-yellow-400", icon: "🟡" },
    { label: "Fort — bon mot de passe", color: "text-blue-700", bar: 80, barColor: "bg-blue-500", icon: "🔵" },
    { label: "Très fort — excellent choix !", color: "text-green-700", bar: 100, barColor: "bg-green-500", icon: "🟢" },
];
...
function getRiskLabel(riskLevel) {
    switch (riskLevel) {
        case "critical":
            return { text: "🔴 Risque critique", tone: "text-red-700 bg-red-100" };
        case "high":
            return { text: "🟠 Risque élevé", tone: "text-orange-700 bg-orange-100" };
        case "medium":
            return { text: "🟡 Risque modéré", tone: "text-yellow-700 bg-yellow-100" };
        case "low":
            return { text: "🟢 Risque faible", tone: "text-green-700 bg-green-100" };
        default:
            return { text: "✅ Aucune fuite détectée", tone: "text-green-700 bg-green-100" };
    }
}
...
function BreachLabPage() {
    ...
    const handleManualCheck = async () => {
        ...
        try {
            const result = await manualCheckMutation.mutateAsync(customPassword);
                                    ...
                                    {scanResult.error ? (
                                        ...
                                        <>
                                            <p className="font-medium text-rose-900">
                                                {scanResult.compromisedDetected > 0
                                            ...
                                            <div className="grid grid-cols-3 gap-2 mt-3 text-center">
                                                <div className="rounded-md border border-rose-200 py-2">
                                                    <p className="text-xs text-rose-700">Mots de passe vérifiés</p>
                                                    <p className="font-semibold text-rose-900">{scanResult.scanned}</p>
                                                </div>
                                                <div className="rounded-md border border-rose-200 py-2">
                                                    <p className="text-xs text-rose-700">Compromis détectés</p>
                                                    <p className="font-semibold text-rose-900">{scanResult.compromisedDetected}</p>
                                                </div>
                                                <div className="rounded-md border border-rose-200 py-2">
                                                    <p className="text-xs text-rose-700">Comptes mis à jour</p>
                                                    <p className="font-semibold text-rose-900">{scanResult.updatedEntries}</p>
                                                </div>
                                            </div>
                                        </>
                                    )}
                            ...
                            {customPassword.length > 0 && (
                                <div className="mt-3">
                                    <div className="flex items-center justify-between mb-1">
                                        <span className="text-xs text-blue-700">Niveau de sécurité</span>
                                    ...
                                    <div className="w-full bg-gray-200 rounded-full h-2">
                                        <div
                                            ...
                                            style={{ width: `${strengthInfo.bar}%` }}
                                        />
                                    </div>
                                    <p className="text-xs text-blue-600 mt-1">
                                        Un pirate aurait besoin de{" "}
                                        <strong>{CRACK_TIME_LABELS[strength - 1] || "quelques secondes"}</strong>{" "}
                                    ...
                                    {manualResult.error ? (
                                        ...
                                        <>
                                            ...
                                            <div className="mt-2 flex flex-wrap items-center gap-2">
                                                ...
                                                >
                                                    {getRiskLabel(manualResult.riskLevel).text}
                                                </span>
                                                {manualResult.breachCount > 0 && (
                                                    <span className="text-xs text-blue-800">
                                                        Vu{" "}
                                                        <strong>{manualResult.breachCount.toLocaleString("fr-FR")}</strong>{" "}
                                                        fois dans des piratages de sites web
                                                    </span>
                                                )}
                                            </div>
                                        </>
                    ...
                    <Card className="bg-linear-to-br from-violet-50 to-fuchsia-50 border-violet-200">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div className="rounded-lg bg-white border border-violet-200 p-4">
                                <p className="text-sm font-semibold text-violet-900">🔒 Votre confidentialité est protégée</p>
                            ...
                            <div className="rounded-lg bg-white border border-violet-200 p-4">
                                <p className="text-sm font-semibold text-violet-900">⚡ Résultats instantanés</p>
                                <p className="text-xs text-violet-700 mt-1">
                                    L&apos;analyse s&apos;appuie sur une base de données de milliards de mots de passe volés lors de piratages réels de sites web.
                                </p>
                            </div>
                            <div className="rounded-lg bg-white border border-violet-200 p-4">
                                <p className="text-sm font-semibold text-violet-900">🎯 Que faire si un mot de passe a fuité ?</p>
                                <p className="text-xs text-violet-700 mt-1">
                        ...
                        <div className="mt-5">
                            <Link href="/generator">
                                <Button variant="secondary">Générer un mot de passe robuste</Button>
                            </Link>
                        </div>
                    </Card>
                </div>
